...
// Also add this to ensure the preview shows on page load
document.addEventListener('DOMContentLoaded', function() {
  updateTopupCostPreview();
  // Set initial value to show preview
  if (document.getElementById('topupDays')) {
    document.getElementById('topupDays').value = '1';
    updateTopupCostPreview();
  }
});
if (document.getElementById('topupDays')) {
...
void setup() {
  ...
  // Fetch backup data from Google Sheets asynchronously so setup is non-blocking
  if (WiFi.status() == WL_CONNECTED) {
    xTaskCreatePinnedToCore(fetchBackupTask, "GoogleFetch", 4096, NULL, 1, NULL, 0);
  }
