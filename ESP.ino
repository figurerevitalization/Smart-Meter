...
const char* GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbxCdne0QgrguqF-EADH2WtX5meJoJ2VwvzsCknYEPsCWKLU4QVVgSrJ-uIY_rHHoY0/exec";
...
void recordBillingSession(unsigned long usedSeconds) {
  ...
  if (billing.sessionStartKwh > 0) {
    ...
    if (rec.energyUsed < 0) rec.energyUsed = 0;
  } else {
    rec.energyUsed = 0;
  }
...
void fetchBackupFromGoogleSheets() {
  ...
  Serial.println("[GOOGLE] All fetch attempts failed - using local EEPROM data");
}
...
void saveDynamicLoads() {
  ...
  for (int i = 0; i < kMaxDynamicLoads; ++i) {
    ...
    if (dynamicLoads[i].active) {
      String data = dynamicLoads[i].name + "," + String(dynamicLoads[i].minW) + "," + String(dynamicLoads[i].maxW);
...
void parseUnoLine(const char* line) {
  ...
  tok = strtok(NULL, ","); if (!tok) return; tele.current    = atof(tok);
  tok = strtok(NULL, ","); if (!tok) return; tele.apparentVA = atof(tok);
  tok = strtok(NULL, ","); if (!tok) return; tele.realW      = atof(tok);
  tok = strtok(NULL, ","); if (!tok) return; tele.reactiveQ  = atof(tok);
  tok = strtok(NULL, ","); if (!tok) return; tele.distortion = atof(tok);
  ...
  tok = strtok(NULL, ","); if (!tok) return; tele.totalKwh      = atof(tok);
  tok = strtok(NULL, ","); if (!tok) return; tele.sessionCostRs = atof(tok);
  tok = strtok(NULL, ","); if (!tok) return; tele.mobileKwh     = atof(tok);
  tok = strtok(NULL, ","); if (!tok) return; tele.laptopKwh     = atof(tok);
  tok = strtok(NULL, ","); if (!tok) return; tele.unknownKwh    = atof(tok);

  tok = strtok(NULL, ","); if (!tok) return;
  strncpy(tele.deviceClass, tok, sizeof(tele.deviceClass) - 1);
  tele.deviceClass[sizeof(tele.deviceClass) - 1] = '\0';

  tok = strtok(NULL, ","); if (!tok) return; tele.efficiency = atof(tok);
  ...
  // ESP32 is authoritative for actual relay control. Keep Uno state in telemetry only.
  // relayState stays under ESP32 control via setRelay().

  // ESP32-side sanity clamping to prevent spikes / invalid values from going to UI
  if (tele.realW < 0.0f) tele.realW = 0.0f;
  if (tele.apparentVA < tele.realW) tele.apparentVA = tele.realW;
  if (tele.current < 0.0f) tele.current = 0.0f;
  if (tele.voltage < 0.0f) tele.voltage = 0.0f;
  ...
  if (tele.analysisActive && tele.realW > 0.5f) {
    ...
    if ((millis() - tele.analysisStartMs) > 2000UL) {
      if (tele.realW < tele.analysisMinVA) tele.analysisMinVA = tele.realW;
      if (tele.realW > tele.analysisMaxVA) tele.analysisMaxVA = tele.realW;
    }
  }
  ...
  if (tele.realW > 0.5f) {
    for (int i = 0; i < kMaxDynamicLoads; ++i) {
      if (!dynamicLoads[i].active) continue;
      float buffer = (dynamicLoads[i].maxW - dynamicLoads[i].minW) * 0.15f + 2.0f; // ±15% + 2W floor
      if (tele.realW >= (dynamicLoads[i].minW - buffer) &&
          tele.realW <= (dynamicLoads[i].maxW + buffer)) {
        strncpy(tele.dynamicDevice, dynamicLoads[i].name.c_str(), sizeof(tele.dynamicDevice) - 1);
...
void handleApiLoadUpdate() {
  ...
  if (id >= 0 && id < kMaxDynamicLoads && dynamicLoads[id].active) {
    ...
    UNO_SERIAL.printf("UPDATELOAD,%d,%s,%.1f,%.1f\n", id, name.c_str(), minW, maxW);
    ok = true;
  }
  if (ok) saveDynamicLoads();
  server.sendHeader("Access-Control-Allow-Origin", "*");
  server.send(200, "application/json", String("{\"ok\":") + (ok ? "true" : "false") + "}");
}
...
void handleApiAnalysisStop() {
  ...
  if (tele.analysisActive && tele.analysisMinVA < 1e5f && tele.analysisMaxVA > 0) {
    // Get the name from the capture input (already in browser)
    String name = server.arg("name");
    if (name.length() == 0) name = "Analysis";
    ...
    for (int i = 0; i < kMaxDynamicLoads; ++i) {
      if (!dynamicLoads[i].active) {
        ...
        UNO_SERIAL.printf("ADDLOAD,%s,%.1f,%.1f\n", name.c_str(), tele.analysisMinVA, tele.analysisMaxVA);
...
void updateBillingTimer() {
  if (!billing.timerRunning) return;
  ...
  if (!billing.oneDayAlertSent && oldRemaining > 86400UL && billing.remainingSeconds <= 86400UL) {
  ...
  if (billing.remainingSeconds == 0) {
    ...
    unsigned long usedSeconds = billing.sessionStartMs ? (millis() - billing.sessionStartMs) / 1000UL : 0;
...
void startBillingTimer() {
  ...
  if (billing.adminLock || billing.timerRunning) return;
  if (billing.remainingSeconds == 0) return;
...
void handleTopup() {
  ...
  if (billing.adminLock) {
    server.send(403, "application/json", "{\"ok\":false,\"error\":\"admin lock active\"}");
    return;
  }

  int seconds = server.arg("seconds").toInt();
  if (seconds <= 0) {
    server.send(400, "application/json", "{\"ok\":false,\"error\":\"invalid seconds\"}");
    return;
  }
  ...
  String response = String("{\"ok\":true,\"remaining_sec\":") + billing.remainingSeconds + "}";
  server.send(200, "application/json", response);
}

void handleTimerStart() { if (requireAdmin()) { startBillingTimer(); server.send(200, "application/json", "{\"ok\":true}"); }}
void handleTimerPause() { if (requireAdmin()) { pauseBillingTimer(); server.send(200, "application/json", "{\"ok\":true}"); }}
void handleTimerStop() { if (requireAdmin()) { stopBillingTimer(); server.send(200, "application/json", "{\"ok\":true}"); }}
...
void handleAdminLock() {
  ...
  saveBillingToEEPROM();
  server.send(200, "application/json", String("{\"ok\":true,\"adminLock\":") + (billing.adminLock ? "true" : "false") + "}");
}
...
void updateLcdDisplay() {
  unsigned long now = millis();
  if (showingIpAddress && (now - lcdIpDisplayStartMs >= LCD_IP_DISPLAY_DURATION_MS)) {
    showingIpAddress      = false;
    calibrationInProgress = true;
    calibrationStartMs    = now;
    lcd.clear();
  }
  if (showingIpAddress) return;
  if (calibrationInProgress && (now - calibrationStartMs < 8000)) {
    ...
    if (now - lastCalibUpdateMs >= 1000) {
      ...
      lcd.setCursor(0, 1);
      char line[17]; snprintf(line, sizeof(line), "Relay OFF %d sec", (int)(rem / 1000));
      lcd.print(line);
    }
    return;
  }
  if (calibrationInProgress) { calibrationInProgress = false; lcd.clear(); }
  if (now - lastLcdUpdateMs < LCD_UPDATE_INTERVAL_MS) return;
  lastLcdUpdateMs = now;
  lcd.setCursor(0, 0);
  char line1[17]; snprintf(line1, sizeof(line1), "V:%3.0f I:%4.2f %3dW", tele.voltage, tele.current, (int)tele.realW);
  lcd.print(line1);
  lcd.setCursor(0, 1);
  char prefix[5];
  ...
  const char* dispDev = (tele.dynamicDevice[0] != '-' && tele.dynamicDevice[0] != '\0') ? tele.dynamicDevice : tele.device;
  char line2[17]; snprintf(line2, sizeof(line2), "%s%-3s", prefix, dispDev);
  lcd.print(line2);
...
<style>
:root{--bg-color:#FFF8F0;--primary-text:#4B2E2B;--accent:#C08552;--secondary:#8C5A3C;--card-bg:#FFFFFF;--border-radius:12px;--transition:all 0.3s ease;--shadow:0 4px 20px rgba(75,46,43,0.05);--shadow-hover:0 8px 30px rgba(75,46,43,0.1)}
*{margin:0;padding:0;box-sizing:border-box;font-family:'Segoe UI',system-ui,sans-serif}
body{background-color:var(--bg-color);color:var(--primary-text);display:flex;height:100vh;overflow:hidden}
.sidebar{width:80px;background-color:var(--card-bg);display:flex;flex-direction:column;align-items:center;padding:2rem 0;box-shadow:2px 0 15px rgba(0,0,0,0.03);z-index:10;flex-shrink:0}
...
.nav-item{position:relative;display:flex;justify-content:center;align-items:center;width:100%;padding:1rem 0;color:var(--secondary);cursor:pointer;transition:var(--transition)}
...
.main-content{flex:1;display:flex;flex-direction:column;overflow-y:auto;padding:2rem 3rem;min-width:0}
...
.relay-toggle{display:flex;align-items:center;gap:1rem;background:var(--card-bg);padding:0.75rem 1.5rem;border-radius:50px;box-shadow:var(--shadow)}
...
.kpi-card{background:var(--card-bg);padding:1.5rem 1rem;border-radius:var(--border-radius);box-shadow:var(--shadow);display:flex;flex-direction:column;align-items:center;justify-content:center;transition:var(--transition)}
.kpi-card:hover{transform:translateY(-5px);box-shadow:var(--shadow-hover)}
.kpi-value{font-size:1.8rem;font-weight:700;color:var(--primary-text);margin-bottom:0.25rem;display:flex;align-items:baseline;gap:0.25rem}
.kpi-unit{font-size:0.9rem;color:var(--accent);font-weight:600}
.kpi-label{font-size:0.8rem;text-transform:uppercase;letter-spacing:1px;color:var(--secondary);font-weight:500}
...
.card-container{background:var(--card-bg);border-radius:var(--border-radius);padding:1.5rem;box-shadow:var(--shadow)}
...
.breakdown-row{display:flex;justify-content:space-between;margin-bottom:1rem;align-items:center}
...
.badge{padding:0.25rem 0.75rem;border-radius:20px;font-size:0.75rem;font-weight:600;background:rgba(192,133,82,0.1);color:var(--accent)}
...
.device-info{font-size:1.5rem;font-weight:700;margin-bottom:0.5rem}
.runtime-cost{margin-top:1.5rem;display:flex;justify-content:space-between}
.runtime-cost>div{flex:1}
.runtime-cost label{font-size:0.8rem;color:var(--secondary)}
.runtime-cost .val{font-weight:600}
.btn-primary{background:var(--accent);color:white;border:none;padding:0.8rem 1.6rem;border-radius:8px;font-weight:700;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;gap:0.5rem;transition:all 0.2s ease;font-size:1rem;box-shadow:0 2px 8px rgba(0,0,0,0.15);min-height:44px;text-decoration:none}
...
th{text-align:left;padding:1rem;color:var(--secondary);font-weight:500;font-size:0.85rem;border-bottom:1px solid rgba(140,90,60,0.1)}
td{padding:0.75rem 1rem;font-size:0.95rem;color:var(--primary-text);border-bottom:1px solid rgba(140,90,60,0.05);vertical-align:middle}
tr:hover td{background:rgba(255,248,240,0.5)}
.status-badge{padding:0.35rem 0.75rem;border-radius:20px;font-size:0.75rem;font-weight:600;background:rgba(192,133,82,0.1);color:var(--accent)}
.status-badge.on{background:rgba(34,197,94,0.1);color:#22c55e}
.load-row{display:flex;align-items:center;gap:0.75rem;padding:0.9rem 1.2rem;background:var(--card-bg);border-radius:10px;box-shadow:var(--shadow);border:1px solid rgba(140,90,60,0.08)}
.load-row input[type=text],.load-row input[type=number]{border:1.5px solid rgba(140,90,60,0.25);border-radius:6px;padding:0.45rem 0.6rem;font-size:0.9rem;color:var(--primary-text);background:var(--bg-color);outline:none}
.load-row input[type=text]:focus,.load-row input[type=number]:focus{border-color:var(--accent)}
.load-row input[type=text]{flex:2;min-width:90px}
.load-row input[type=number]{width:82px}
.load-row .load-label{font-size:0.75rem;color:var(--secondary);font-weight:500;white-space:nowrap}
.analysis-banner{padding:1.25rem 1.5rem;background:rgba(34,197,94,0.08);border:1.5px solid #22c55e;border-radius:10px;display:flex;align-items:center;gap:1rem;margin-bottom:1.5rem}
...
@media(max-width:1024px){.dashboard-grid{grid-template-columns:1fr}.kpi-row{grid-template-columns:repeat(3,1fr)}}
@media(max-width:768px){.main-content{padding:1rem}.kpi-row{grid-template-columns:repeat(2,1fr)}.top-bar{flex-direction:column;gap:1rem;align-items:flex-start}.top-right{width:100%;justify-content:space-between}}
</style>
</head>
<body>
  ...
  <div id="dashboardPane">
    ...
    <header class="top-bar">
      ...
      <div class="top-right">
        ...
        <div style="display:flex;gap:0.5rem;align-items:center;margin-left:1rem;flex-wrap:wrap">
          <select id="topupPreset" style="padding:0.4rem;border:1px solid rgba(140,90,60,0.25);border-radius:6px;font-size:0.85rem;" onchange="syncTopupFromPreset()">
          ...
          <input id="topupDays" type="number" min="1" max="90" placeholder="days" style="width:70px;padding:0.4rem;border:1px solid rgba(140,90,60,0.25);border-radius:6px;font-size:0.85rem;" oninput="document.getElementById('topupPreset').value=''">
          <button class="btn-primary" style="font-size:0.8rem;padding:0.5rem 0.8rem;" onclick="topup()">Pay & Start</button>
        </div>
        <div id="topupCostPreview" style="font-size:0.8rem;color:var(--secondary);margin-top:0.3rem"></div>
      </div>
    </header>
  ...
  <div id="analyticsPane" style="display:none;">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.5rem;margin-bottom:1.5rem">
      ...
      <div class="card-container">
        ...
        <div style="position:relative;height:150px"><canvas id="an_cV"></canvas></div>
      </div>
      ...
      <div class="card-container">
        ...
        <div style="position:relative;height:150px"><canvas id="an_cI"></canvas></div>
      </div>
      ...
      <div class="card-container">
        ...
        <div style="position:relative;height:150px"><canvas id="an_cP"></canvas></div>
      </div>
      ...
      <div class="card-container">
        ...
        <div style="position:relative;height:150px"><canvas id="an_cS"></canvas></div>
      </div>
      ...
      <div class="card-container">
        ...
        <div style="position:relative;height:150px"><canvas id="an_cQ"></canvas></div>
      </div>
      ...
      <div class="card-container">
        ...
        <div style="position:relative;height:150px"><canvas id="an_cPF"></canvas></div>
      </div>
    ...
    <div class="card-container" style="margin-bottom:1.5rem">
      ...
      <div style="position:relative;height:200px"><canvas id="an_cDI"></canvas></div>
    </div>
    ...
    <div class="card-container" style="margin-bottom:1.5rem">
      ...
      <div style="position:relative;height:200px"><canvas id="an_cKWH"></canvas></div>
    </div>
...
<script>
var systemData={v:0,i:0,s:0,p:0,q:0,di:0,pf:0,kwh:0,cost:0,relay:false,dev:"-",lt:"-",dynamic:"-",dynamicLoads:[],age_ms:9999};
var isAdmin=false;
...
function syncTopupFromPreset() {
  ...
  if (val) document.getElementById('topupDays').value = val;
}

async function topup(){
  var days = parseFloat(document.getElementById('topupDays').value);
  if (!(days >= 1 && days <= 90)) { showBlocked('Invalid top-up','Enter 1–90 days'); return; }
  var seconds = Math.round(days * 86400);
  var res = await fetch('/api/topup?seconds=' + seconds);
  if (res.status === 403) {
    showBlocked('Top-up blocked', 'Admin lock active');
    return;
  }
  if (!res.ok) {
  ...
  document.getElementById('topupDays').value = '';
  document.getElementById('topupPreset').value = '';
  document.getElementById('topupCostPreview').innerText = '';
  await fetchData();
}
...
setInterval(refreshBillingStatus, 5000);
...
function updateTopupCostPreview() {
  var days = parseFloat(document.getElementById('topupDays').value) || 0;
  var costPerHour = systemData.billing_cost_per_hour || 5.0;
  var el = document.getElementById('topupCostPreview');
  if (!el) return;
  if (days > 0) {
    var cost = days * 24 * costPerHour;
    el.innerText = 'Estimated cost: ₹' + cost.toFixed(2);
  } else {
    el.innerText = '';
  }
}
if (document.getElementById('topupDays')) {
  document.getElementById('topupDays').addEventListener('input', updateTopupCostPreview);
}
...
void setup() {
  ...
  // Fetch backup data from Google Sheets if connected (with delay for stability)
  if (WiFi.status() == WL_CONNECTED) {
    delay(3000);  // Wait 3 seconds for IP to stabilize
    fetchBackupFromGoogleSheets();
  }
  
  // Ensure relay is in correct state after loading EEPROM
  if (billing.adminLock || billing.remainingSeconds == 0 || !billing.timerRunning) {
    setRelay(false);
  } else {
    setRelay(true);
  }
...
void loop() {
  ...
  while (UNO_SERIAL.available()) {
    ...
    if (c == '\n' || c == '\r') {
      if (pos > 4) { rawLine[pos] = '\0'; parseUnoLine(rawLine); }
      pos = 0;
  ...
  if (millis() - lastWiFiCheck > 10000) {
    ...
    if (WiFi.status() != WL_CONNECTED) { Serial.println("WiFi dropped — reconnecting…"); WiFi.reconnect(); }
  }

  // Upload live data to Google Sheets every 5 minutes
  static unsigned long lastUpload = 0;
  if (millis() - lastUpload > 300000 && WiFi.status() == WL_CONNECTED) {  // 5 minutes
    lastUpload = millis();
